...
from Model.Model import QNetwork, GenerativeGaussianMLPActor, GaussianPolicy, DeterministicPolicy, GenerativeGaussianMLPActor2, MLPQFunction
import itertools
import os
import time
...
class GAC(object):
    def __init__(self, state_dim, action_dim, args):
        ...
        self.critic1 = MLPQFunction(state_dim, action_dim, args.hidden_dim).to(device=self.device)
        self.critic2 = MLPQFunction(state_dim, action_dim, args.hidden_dim).to(device=self.device)
        # #지워라 나중에~~~~~~~~~~~
        # self.critic1.load_state_dict(torch.load("q1.pth"))
        # self.critic2.load_state_dict(torch.load("q2.pth"))
        ...
        self.critic1_target = MLPQFunction(state_dim, action_dim, args.hidden_dim).to(device=self.device)
        self.critic2_target = MLPQFunction(state_dim, action_dim, args.hidden_dim).to(device=self.device)
        ...
        # # 지워라 나중에~~~~~~~~~~~
        # self.policy.load_state_dict(torch.load("pi.pth"))
    ...
    def update_parameters(self, batch_size):
        ...
        qf1_pi = self.critic1(state_batch_repeat, pi_repeat)
        qf2_pi = self.critic2(state_batch_repeat, pi_repeat)
